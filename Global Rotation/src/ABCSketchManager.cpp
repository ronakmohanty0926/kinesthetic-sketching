			...
			if (!isnan(angle))
			{
				...
				for (int i = 0; i < Sketch.size(); i++)
				{
					//Sketch[i].Rotate(angle, axis, Origin);
						...
						{
							Sketch[i].GetVertex(j, q);
							RotateVec3(q, matrix);
							Sketch[i].SetVertex(q, j);
...
bool ABCSketchManager::SaveSketchData()
{
	...
	{
		ofstream myfile;
		myfile.open("Global_SketchData.txt");
		int t = 0;
