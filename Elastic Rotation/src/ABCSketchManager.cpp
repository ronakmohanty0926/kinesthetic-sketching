...
{
	start = clock();
	hapticstylusmov.push_back(start);
...
void ABCSketchManager::UpdateSketch()
{
	...
	else if (mode == MODE_ROTATION)
	{
		...
		//globalaxis[] = { axis[0], axis[1], axis[2] };
		//int vertices = (int)(Sketch[Sketch.size() - 1].GetNumVertices);

		bool b = Normalize3(axis);
		if (b && !Sketch.empty())
		{
			angle = 0.004*Norm3(_traj);
			...
			for (int i = 0; i < Sketch.size(); i++)
			{
				...
				{
					float s[3], z[3];
					Sketch[i].GetVertex(j, s);
					RotateVec3(s, matrix);
					Sketch[i].SetVertex(s, j);
				}
			}
		}
	}
...
ABCSketchManager::ABCSketchManager()
{
	...
	//Cube
	//Eight vertices in the meshVert, create an edge list
	Tuple2i t;
...
{	
	if (!Sketch.empty())
	{
		_pivot[0] = prev_stylus[0];
		_pivot[1] = prev_stylus[1];
		_pivot[2] = prev_stylus[2];		
		mode = MODE_ROTATION;
		SortSketchZ();
		return true;
	}
...
bool ABCSketchManager::SaveLog()
{
	if (!Sketch.empty())
	{	ofstream myfile;
		myfile.open("Elastic_UserLOG.txt");
...
bool ABCSketchManager::SaveSketchData()
{
	...
	{
		ofstream myfile;
		myfile.open("Elastic_SketchData.txt");
		int t = 0;
...
bool ABCSketchManager::SaveMeshData()
{
	...
	{
		ofstream myfile;
		myfile.open("Elastic_MeshData.txt");
		int t = 0;
		myfile << "X" << " " << "Y" << " " << "Z" << endl;
...
bool ABCSketchManager::SaveMetric()
{
	if (!Sketch.empty())
	{
		ComputeError();
		ofstream myfile;
		myfile.open("Elastic_Metric.txt");
