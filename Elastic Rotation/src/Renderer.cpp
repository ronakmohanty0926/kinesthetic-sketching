...
namespace midl {
	...
	void OrthographicView::SetParameters(float prj_right, float prj_left, float prj_top, float prj_bottom, float near prj_near, float far prj_far)
	{
		this->prj_right = prj_right;
		this->prj_left = prj_left;
		this->prj_top = prj_top;
		this->prj_bottom = prj_bottom;
		this->prj_near = prj_near;
		this->prj_far = prj_far;
	}

	void OrthographicView::SetCameraEye(float x, float y, float z)
	{
		this->eye[0] = x;
		this->eye[1] = y;
		this->eye[2] = z;
	...
	void OrthographicView::SetCameraEye(float *eye)
	{
		this->eye[0] = eye[0];
		this->eye[1] = eye[1];
		this->eye[2] = eye[2];
	...
	void OrthographicView::SetCameraCenter(float x, float y, float z)
	{
		this->center[0] = x;
		this->center[1] = y;
		this->center[2] = z;
	...
	void OrthographicView::SetCameraCenter(float *center)
	{
		this->center[0] = center[0];
		this->center[1] = center[1];
		this->center[2] = center[2];
	...
	void OrthographicView::SetCameraHead(float x, float y, float z)
	{
		this->head[0] = x;
		this->head[1] = y;
		this->head[2] = z;
	...
	void OrthographicView::SetCameraHead(float *head)
	{
		this->head[0] = head[0];
		this->head[1] = head[1];
		this->head[2] = head[2];
	...
	void OrthographicView::PixelToWindow(int px, int py, float *windowCoordinates)
	{
		windowCoordinates[0] = (float)px;
		windowCoordinates[1] = (float)(viewport[3] - py);
	}

	void OrthographicView::PixelToNormalized(int px, int py, float *normalizedCoordinates)
	{
		normalizedCoordinates[0] = -0.5 + (float)px / (float)viewport[2];
		normalizedCoordinates[1] = -0.5 + (float)(viewport[3] - py);
	}

	void OrthographicView::PixelToWorld(int px, int py, float depth, float *worldCoordinates)
	{
		GLfloat winX, winY;
		winX = (float)px;
		winY = (float)(viewport[3] - py);
	...
	void OrthographicView::PixelToRay(int px, int py, float *rayStart, float *rayEnd)
	{
		GLfloat winX, winY;
		winX = (float)px;
		winY = (float)(viewport[3] - py);
	...
	void OrthographicView::Reshape(int w, int h)
	{
		glViewport(0, 0, (GLsizei)w, (GLsizei)h);
		...
		//gluPerspective(prj_fov, (GLfloat)w / (GLfloat)h, prj_near, prj_far);
		glOrtho(prj_left*((GLfloat)w / (GLfloat)h), prj_right*((GLfloat)w / (GLfloat)h), prj_bottom, prj_top, prj_near, prj_far);
		glMatrixMode(GL_MODELVIEW);
	...
	void OrthographicView::Bind()
	{
		glLoadIdentity();
		gluLookAt(eye[0], eye[1], eye[2], center[0], center[1], center[2], head[0], head[1], head[2]);
	}

	void OrthographicView::Unbind()
	{
		glGetDoublev(GL_PROJECTION_MATRIX, projectionMatrix);
		glGetDoublev(GL_MODELVIEW_MATRIX, modelMatrix);
		glGetIntegerv(GL_VIEWPORT, viewport);
